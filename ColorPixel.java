...
public class ColorPixel implements Pixel {
	...
	public Pixel blend(Pixel p, double weight) {
		...
		double newRed = (weight * p.getRed()) + ((1 - weight) * this.red);
		double newGreen = (weight * p.getGreen()) + ((1 - weight) * this.green);
		double newBlue = (weight * p.getBlue()) + ((1 - weight) * this.blue);

		Pixel blendPixel = new ColorPixel(newRed, newGreen, newBlue);
		return blendPixel;
	}
